...
JsonRoutes.add("get", "/fhir/MedicationOrder", function (req, res, next) { process.env.DEBUG && console.log('GET /fhir/MedicationOrder', req.query);
  ...
  if (accessToken || process.env.NOAUTH) {
    ...
    process.env.DEBUG && console.log('MedicationOrders.find(id).count()', MedicationOrders.find(databaseQuery).count()); 

    // MedicationOrders returns an object instead of a pure JSON document
    ...
    medicationOrderData.forEach(function(medicationOrder){
